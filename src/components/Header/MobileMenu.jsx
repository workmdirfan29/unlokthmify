...
const MobileMenu = ({
  ...
  return (
    <>
      <div
        className={`fixed top-0 right-0 z-50 bg-white flex flex-col items-start p-4 transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? "transform translate-x-0" : "transform translate-x-full"
          }`}
      ...
      >
        <div className="flex items-center justify-between w-full">
          ...
          <button onClick={toggleMobileMenu} aria-label="Close Mobile Menu" className="text-3xl">
            <FaTimes />
          </button>
        </div>
      ...
      {isMobileMenu && (
        <div className="absolute top-0 right-0 flex items-center p-2 pr-8 md:hidden">
          <button onClick={toggleMobileMenu} aria-label="Toggle Mobile Menu">
            {isMobileMenuOpen ? <FaTimes className="text-3xl" /> : <RiMenu3Line className="text-4xl" />}
          </button>
        </div>
      )}
    </>
